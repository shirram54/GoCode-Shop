import "./About.css";

function About() {
  return (
    <div className="abocen">
      <h3>
        WE ARE <span className="purple">GOCODE SHOP</span>
      </h3>
      At GoCode Shop we bring the most worthwhile items From the right brands
      and from all over the world.
      <br />
      <br />
      What is hot today can be worn tomorrow.
      <br />
      <br />
      No need to wait. Do not need favors.
      <br />
      <br />
      GoCode Shop lands you in the heart of the fashion world. You are welcome
      to take off. No need to fasten belts.
      <br />
      <br />
      We live in a crazy pace of changing fashion.
      <br />
      <br />
      GoCode Shop carefully selects brands and collections From everything the
      fashion world has to offer.
      <br />
      <br />
      In GoCode Shop no satisfactions are rejected. Buy today and wear tomorrow.
      <br />
      <br />
      The collections change at a fast pace And what's on the site today, is not
      sure will be there tomorrow.
      <br />
      <br />
      New products are updated on the site every day. Everything is new, all the
      time.
      <br />
      <br />
      <br />
      <img
        src="https://static.zara.net/photos///2021/I/0/1/p/9342/928/705/5/w/2174/9342928705_15_2_1.jpg?ts=1637312181911"
        alt="im"
      ></img>
    </div>
  );
}
export default About;
